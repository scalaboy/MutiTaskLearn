...
from mctr.data.ali_ccp_datasets import get_dataset, get_dataset_mof
...
from mctr.modeling.cgc import CGC
...
if model_name in ['esmm_mmoe_add_loss', 'esmm_mmoe','mmoe_add_loss']:
...
def get_prefix(paras=None):
    ...
    if paras:
        ...
        f_dict = {
            'model': model_name +  '-'+str(time.time().as_integer_ratio()[0]) + '_cgc' + '-'.join(
...
def get_cgc_add_loss():
    mmoe_layers = CGC(units=units, num_shared_experts=num_experts, num_tasks=num_tasks,num_task_experts_list=[1, 0, 3, 2])(din_ctr)
    ctr_pred = EsmmMLPLayer(name='ctr', output_name='ctr_output', dense_shapes=mlp_ctr, batch_norm=batch_norm,
    ...
    ctcvr_pred = layers.Lambda(lambda x: (0.05 + x[1]-0.05*x[2])/1.05, name='ctcvr')([ctr_pred, cvr_pred,ct_nocvr_pred])
    return ctr_pred, cvr_pred,ctcvr_pred,  ct_nocvr_pred
...
def build_model(name):
    m_dict = {
        'esmm': get_esmm,
        'esmm_mmoe': get_mmoe,
        'esmm_mmoe_add_loss': get_cgc_add_loss,
    }
    return m_dict[name]()
...
def train_mmoe_add_loss():
    ...
    ctr_pred, cvr_pred, ctcvr_pred, ct_nocvr_pred = get_cgc_add_loss()
    ...
    model.compile(optimizer=opt,
                  ...
                  metrics={
                      ...
                      'ctcvr_pred': [DropAUCX(name='dauc', output_name='ctcvr', filter=True), DropAUC(name='tauc',filter=True)],
                      'cvr_output': [DropAUCX(name='dauc', output_name='cvr', filter=True),
                                     DropAUC(name='tauc', filter=True)],
                      'ct_nocvr_pred': [DropAUCX(name='dauc', output_name='ct_nocvr'), DropAUC(name='tauc')]
                  },
                  # steps_per_execution=10
                  )
    print('start fitting', prefix)
    train_dataset = get_dataset_mof(batch_size=batch_size, train=True, config=config)
    valid_dataset = get_dataset_mof(batch_size=batch_size, train=False, config=config)
