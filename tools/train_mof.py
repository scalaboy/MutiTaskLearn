...
def get_prefix(paras=None):
    ...
    if paras:
        ...
        f_dict = {
            'model': model_name +  '-'+str(time.time().as_integer_ratio()[0]) + '_mof' + '-'.join(
...
def get_mmoe_add_loss():
    ...
    #ctcvr_pred = layers.Lambda(lambda x: (0.08 + x[1]-0.08*x[2])/1.08, name='ctcvr')([ctr_pred, cvr_pred,ct_nocvr_pred])
    ctcvr_pred = layers.Lambda(lambda x: (0.04 + x[1] - 0.04 * x[2]) / 1.04, name='ctcvr')(
        [ctr_pred, cvr_pred, ct_nocvr_pred])
